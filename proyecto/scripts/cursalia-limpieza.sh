...
bleach_root() {
  if command -v bleachbit >/dev/null 2>&1; then
    ...
    xfce4-terminal --title="BleachBit (administrador)" -e "bash -c 'echo Abriendo BleachBit como administrador...; echo; sudo -E bleachbit; echo; echo Puedes cerrar esta ventana.'" &
  else instalar_bleach; fi
}
...
menu() {
  ...
  yad --class=cursaliacentro --title="Limpieza cursalialinux" --center --width=560 --height=460 \
